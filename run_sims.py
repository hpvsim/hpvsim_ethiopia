"""
Define an HPVsim simulation for Ethiopia, including calibration
"""
...
def make_sim(location='ethiopia', calib_pars=None, debug=0, interventions=None, analyzers=None, datafile=None, seed=1, end=2020):
    """
    ...
    # Debut: derived by fitting to 2016 DHS
    # Women:
    #           Age:   15,   18,   20,   22,   25
    #   Prop_active:   24, 62.3, 75.9, 84.1, 90.2
    # Men:
    #           Age:  15,   18,   20,   22,   25
    #   Prop_active: 1.8, 17.0, 36.4, 56.3, 75.8
    # For fitting, see https://www.researchsquare.com/article/rs-3074559/v1
    pars.debut = dict(
        f=dict(dist='lognormal', par1=16.77, par2=2.42),
        m=dict(dist='lognormal', par1=np.nan, par2=np.nan),
    )
    ...
    pars.layer_probs = dict(
        m=np.array([
            ...
            [0, 0, 0.035, 0.1505, 0.4515, 0.5852, 0.6195, 0.6041, 0.6034, 0.5691, 0.385, 0.28, 0.28, 0.28, 0.28, 0.28],
            [0, 0, 0.01, 0.215, 0.645, 0.836, 0.885, 0.863, 0.862, 0.813, 0.7, 0.8, 0.7, 0.6, 0.5, 0.6],
        ]),
        c=np.array([
            # 0, 5,  10,  15,  20,  25,  30,  35,  40,  45,  50,  55,  60,  65,   70,   75
            [0, 5, 10,    15,     20,     25,     30,     35,     40,     45,   50,   55,   60,   65,    70,   75],
...
def run_calib(n_trials=None, n_workers=None, do_save=True, filestem=''):
    ...
    calib = hpv.Calibration(sim, calib_pars=calib_pars, genotype_pars=genotype_pars,
                            name=f'ethiopia_calib',
                            datafiles=datafiles,
                            total_trials=n_trials, n_workers=n_workers,
                            storage=storage
                            )
    calib.calibrate()
    filename = f'ethiopia_calib{filestem}'
    if do_save:
...
def plot_calib(which_pars=0, save_pars=True, filestem=''):
    ...
    if save_pars:
        ...
        sc.save(f'results/ethiopia_pars{filestem}.obj', calib_pars)
        sc.save(f'results/ethiopia_pars{filestem}_all.obj', trial_pars)
...
def run_parsets(debug=False, verbose=.1, analyzers=None, save_results=True, **kwargs):
    ...
    parsets = sc.loadobj(f'results/ethiopia_pars_all.obj')
    ...
    msim.reduce()
    if save_results:
        sc.saveobj(f'results/ethiopia_msim.obj', msim.results)
...
if __name__ == '__main__':
    ...
    if 'plot_calib' in to_run:
        ...
        sc.saveobj(f'results/ethiopia_calib_reduced.obj', calib)
